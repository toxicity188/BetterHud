...
fun <T, R> ((T) -> R).memoize(initialValue: R): (T) -> R = this as? MemoizedFunction ?: MemoizedFunction(this, initialValue)
fun <P, T, R> ((P) -> (T) -> R).memoizeBuilder(initialValue: R): (P) -> (T) -> R = { p ->
    this@memoizeBuilder(p).memoize(initialValue)
}
