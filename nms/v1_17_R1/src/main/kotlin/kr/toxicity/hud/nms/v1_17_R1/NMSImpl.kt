...
class NMSImpl : NMS {
    ...
    override fun registerCommand(module: CommandModule<BetterCommandSource>) {
        val dispatcher = (Bukkit.getServer() as CraftServer).handle.server.commands.dispatcher
            ...
            fun changeName(targetBuf: FriendlyByteBuf = buf) {
                runCatching {
                    ...
                    fun applyFont(component: Component): Component {
                        return component.style(Style.style().font(key).build()).children(component.children().map {
                            applyFont(it)
                        })
                    }
